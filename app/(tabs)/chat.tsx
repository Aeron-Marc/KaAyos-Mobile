...
export default function ChatListScreen() {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      const grouped = msgs.reduce<Record<number, { name: string; msgs: Message[] }>>((acc, m) => {
        const otherId = m.sender_id === userId ? m.receiver_id : m.sender_id;
        const otherName = m.sender_id === userId ? m.receiver_name : m.sender_name;
        if (!acc[otherId]) {
          acc[otherId] = { name: otherName, msgs: [] };
        }
      ...
      const convos = Object.entries(grouped).map(([id, data]) => {
        ...
        return {
          ...
          unread: sorted.filter(m => m.receiver_id === userId && !m.read_at).length,
